function Get-PSqliteDBMetadata
{
    <#
    .SYNOPSIS
    ...
    try
    {
        $metadataPresent = Invoke-PSqliteQuery -SqliteConnection $SqliteConnection -CommandText 'SELECT name from sqlite_schema WHERE name = @name COLLATE NOCASE' -Parameters @{name = '_metadata'}
        if ($metadataPresent)
        {
            if ($MetadataKey -contains '*')
            {
                $query = 'SELECT key, value from _metadata;'
                $metadata = Invoke-PSqliteQuery -SqliteConnection $SqliteConnection -CommandText $query -As OrderedDictionary
            }
            else
            {
