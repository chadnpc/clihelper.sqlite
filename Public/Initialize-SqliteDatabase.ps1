...
function Initialize-SqliteDatabase {
  ...
  switch ($PSCmdlet.ParameterSetName) {
    'byPath' {
      if (!(Test-Path -Path $Path -PathType Leaf -IsValid)) {
        ...
        $DatabaseConfig = Get-SqliteDBConfig -ConfigFile $Path
      }
    }
  ...
  if (!$DatabaseConfig.databaseExists()) {
    ...
    $compareResult = Compare-SqliteDBVersion -ExpectedVersion $DatabaseConfig.DBVersion -DatabaseConfig $DatabaseConfig
  ...
  switch ($MigrationMode) {
    ...
    'OVERWRITE' {
      if ($Force.IsPresent -eq $true -or $shouldUpdateDB -eq $true) {
        ...
        # if ($Force.IsPresent -eq $false -and $shouldUpdateDB -eq $false)
